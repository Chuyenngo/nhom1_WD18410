...
class BillDAO
{
    ...
    public function createBill($userId, $number_phone, $address, $email, $full_name,  $create_at)
    {


        $query = "INSERT INTO `bill`( `id_user`, `number_phone`, `address`, `email`, `full_name`,`create_at`) 
            VALUES ('$userId','$number_phone','$address','$email','$full_name','$create_at')";
    ...
    public function showBill($id_user)
    {
        ...
        $query = "SELECT id_bill, id_user, number_phone, address, email, full_name, create_at
    ...
    {
        $query = "SELECT id_bill, quantity, name_product, price_product, id_product, status, returned, type_payment FROM bill_details WHERE id_user = $id_user";
        $stmt = $this->PDO->prepare($query);
        // die($query);
    ...
    public function typePayment($type, $id_bill)
    {
        $query = "UPDATE `bill_details` SET `type_payment`= $type WHERE id_bill= $id_bill";
        $stmt = $this->PDO->prepare($query);
        $stmt->execute();
    }
