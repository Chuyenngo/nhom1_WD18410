...
class BillController
{
    ...
    public function add()
    {
        if (isset($_SESSION["user"])) {
            ...
            // Create the bill
            $this->BillDAO->createBill($id_user, $phone, $address, $email, $full_name, $create_at);
    ...
    {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            // Retrieve the payment method value
            $paymentMethod = $_POST['payment_method'];

            // Use $paymentMethod to determine the payment method and branch accordingly
            if ($paymentMethod === 'cod' || $paymentMethod === 'momo') {
                $type = ($paymentMethod === 'cod') ? 1 : 2;
                ...
                try {
                    ...
                    foreach ($cartItems as $cartItem) {
                        ...
                        if ($availableQuantity >= $quantityInCart) {
                            ...
                            // If there is not enough quantity, remove the item from the cart
                    ...
                    // Update the payment type in the bill_details table
                    $this->BillDAO->typePayment($type, $id_bill);
                    ...
                    if ($paymentMethod === 'cod') {
                        ...
                        require_once('view/bill/user/init_payment.php');
                    }
                    // If all checks pass, delete items from the cart
